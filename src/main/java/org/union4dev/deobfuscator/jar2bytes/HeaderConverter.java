...
public class HeaderConverter {
	...
	public static void processInputJar(String inputPath, String outputPath) throws IOException {
		...
		try (JarFile jarFile = new JarFile(inputPath)) {
			for (ZipEntry entry : jarFile.stream().toList()) {
		...
		Logger.info("Processing " + classes.size() + " classes");
		...
		Logger.info("Load priority calculated, sorting...");
		...
		for (int i = 0; i < classDataStrings.size(); i++) {
			resultFile.append("inline const jbyte class_data_").append(i).append("[] = ").append(classDataStrings.get(i)).append(";\n");
		}
		resultFile.append("\ninline const jbyte* jar_classes_data[] = { ");
		for (int i = 0; i < classDataStrings.size(); i++) {
			resultFile.append("class_data_").append(i).append(", ");
		}
		resultFile.append("};\n\ninline const jint jar_classes_sizes[] = {");
		for (ClassFile file : resultClasses) {
